...
# --- EMBEDDED STYLING FROM HUGGING FACE GREEN COLOR THEME ---
st.markdown("""
    <style>
    .stApp {
...
DISEASE_GUIDE = {
    ...
    'Corn___Common_rust_': {
        ...
        'solution': 'Typically doesn\'t cause severe loss; use resistant hybrids in the future. In severe cases or seed corn production, apply preventative strobulurin or triazole fungicides.'
    },
...
# 3. Load Model with Caching (Adding compile=False for cloud server load compatibility)
@st.cache_resource
def load_my_model():
...
# 4. User Interface Setup (Incorporating original layout data from README.md)
st.markdown("<h1 class='main-title'>🌿 Plant Disease Detector</h1>", unsafe_allow_html=True)
st.write("A deep learning web app that classifies plant leaf diseases from images.")

# Displaying model specifications directly inside an interactive panel
with st.expander("📊 Model Documentation & Technical Specs", expanded=False):
    st.markdown("""
...
# Instructions context block
st.write("### How to Use")
st.markdown("""
1. Upload a clear photo of a plant leaf below.
2. The model will run diagnostic inference automatically.
3. View the classified disease variant alongside precision confidence metrics.
""")
...
if uploaded_file is not None:
    ...
    # Simple lookup block safely containing potential mapping discrepancies
    try:
        diagnostic_info = DISEASE_GUIDE[raw_class_name]
        display_name = diagnostic_info['name']
        display_solution = diagnostic_info['solution']
    except KeyError:
        display_name = raw_class_name.replace("___", " - ").replace("_", " ")
        display_solution = "Remedy steps not found for this specific class index mapping in current directory dictionary."
    
    # 7. Rendering Dynamic UI Components Based on Results
    st.write("---")
    
    if "healthy" in raw_class_name.lower():
        st.success(f"### Diagnostic Result: **{display_name}**")
    else:
        st.error(f"### Diagnostic Result: **{display_name}**")
    ...
    st.info(display_solution)

# --- ACADEMIC METADATA FOOTER FROM README.MD ---
st.markdown("""
    <div class='project-footer'>
        <strong>Tech Stack:</strong> TensorFlow / Keras | Streamlit | Hugging Face Spaces<br>
        Field Work Project — 4th Semester | B.Sc. Computer Science | Utkal University
    </div>
""", unsafe_allow_html=True)
